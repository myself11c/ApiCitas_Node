...
var sedes = {
    ...
    listar: function (request, reply) {
        try {
            var params = [];
            sql_server.storedProcedure("ApiCitas.usp_Sedes", params,  function (err, data) {
                let Response = {};
                
                if (typeof err == "undefined" || err == null) {
                    if(data.recordset != null && data.recordset.length>0){
                        Response.ListaSedes = data.recordset;
                        Response.Error = false;
                        Response.Estado = 200;
                        Response.Mensaje = "Lista de sedes.";
                    }
                    else{
                        Response.ListaSedes = null;
                        Response.Error = true;
                        Response.Estado = 404;
                        Response.Mensaje = "No existen sedes.";
                    }
                }else {
                    Response.ListaSedes = null;
                    Response.Error = true;
